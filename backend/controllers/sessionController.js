...
const evaluateAnswerAsync = async (io, userId, sessionId, questionIndex, audioFilePath = null, code = null) => {
    ...
    console.log('Parameters: questionIndex =', questionIndex, 'audioFilePath =', audioFilePath, 'code =', code);
    ...
    if (!question) {
        console.error(`Question at index ${questionIdx} not found in session`);
    ...
    try {
        ...
        console.log('Sending body to AI service:', {
        ...
        console.log('AI service response status:', evalResponse.status);

        if (!evalResponse.ok) throw new Error('AI Evaluation service failed');

        const evalData = await evalResponse.json();
        console.log('AI service evaluation data parsed:', evalData);
...
const submitAnswer = asyncHandler(async (req, res) => {
    console.log('submitAnswer endpoint hit!');
...
const endSession = asyncHandler(async (req, res) => {
    console.log('endSession endpoint hit! sessionId:', req.params.id);
    const sessionId = req.params.id;
    const userId = req.user._id;
    ...
    if (isProcessing) {
        console.error('Cannot end interview while AI is processing answers.');
        res.status(400);
        throw new Error('Cannot end interview while AI is processing answers.');
    }
    if (session.status === 'completed') {
        console.error('Session is already completed.');
        res.status(400);
    ...
    await session.save();
    console.log('Session saved successfully as completed!');
